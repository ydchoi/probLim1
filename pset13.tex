...
\eQb
\mathbb{E}[Z_{n-1}| \mathscr{F}_n] 
= \dfrac{1}{\prod_{m=1}^{n} (1+Y_m)} \mathbb{E}[X_{n+1}|\mathscr{F}_n] 
\leq 
\dfrac{1}{\prod_{m=1}^{n} (1+Y_m)} X_n = Z_n. 
\eQe
...
\eQb
\prod_{m=1}^{n}(1+Y_m) \>\>\> \text{converges to a finite limit a.s.} 
\eQe
and hence
\eQb
...
\eQb
\text{Var}[S_{\tau}] &=& \mathbb{E}[\xi_1] \mathbb{E}[\tau].
\eQe 
Suppose for a moment that $\tau$ is bounded by some constant $M$. Then, by (a),
\eQb
\text{Var}(S_{\tau}) &=& \mathbb{E}[S_{\tau}^2] = \mathbb{E}[\sum_{i=1}^{n}
\xi_i 1_{\{i \leq \tau\}} )^2] \\
&=& \mathbb{E}[\sum_{i=1}^{M}(\xi_i^2 1_{\{i \leq \tau\}}) = \mathbb{E}[\xi_1^2]
\mathbb{E}[\tau]
\eQe
...
\text{(a)} Let $\lambda > 0$. First of all, $\{M_n\}$ is a martingale, since
\eQb
\mathbb{E}[M_{n+1}| \mathscr{F}_n] &=& \mathbb{E}[e^{\lambda S_{n+1}}
M(\lambda)^{-(n+1)} | \mathscr{F}_n] = M(\lambda)^{-(n+1)}
\mathbb{E}[e^{\lambda S_n} e^{\lambda \xi_{n+1}} | \mathscr{F}_n] \\
&=& M(\lambda)^{-(n+1)} e^{\lambda S_n} \mathbb{E}[e^{\lambda \xi_{n+1}}] 
= e^{\lambda S_n} M(\lambda)^{-n}
\eQe
for each $n \geq 1$. Now, $\{\tau_z \wedge n\}$ are bounded stopping times, so
by theorem 5.4.1 in Durrett,
\eQb
\mathbb{E}[M_{n \wedge \tau_z}] = \mathbb{E}[M_1] = \mathbb{E}[e^{-\lambda \xi_1}
e^{\lambda \xi_1}] = 1
\eQe 
for each $n \geq 1$. Observe that $\mathbb{E}|M_{n \wedge \tau_z}| \leq
e^{\lambda z} M(\lambda)^{-n} < \infty $ for all $n \geq 1$, so by DCT
\eQb
\mathbb{E}[M_{n \wedge \tau_z} \to \mathbb{E}[M_{\tau_z}] \>\>\> \text{as} 
\>\>\> n \to \infty
\eQe
and hence
\eQb
