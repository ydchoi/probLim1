...
\eQb
e^{-X} = e^{-Y} \>\>\> \text{ in distribution}.
\eQe
It follows that, as $e^{-x}$ is strictly decreasing,
\eQb
\mathbb{P}(X \leq t) = \mathbb{P}(e^{-X} \geq e^{-t}) = 
\mathbb{P}(e^{-Y} \geq e^{-t}) = \mathbb{P}(Y \leq t) 
\eQe
...
\eQb
\E[e^{-t_0 X_n}] &\geq& 1 - \dfrac{\epsilon}{2}
\eQe
for any $n \geq N$. 
By monotonicity of $e^{-x}$, 
\eQb
\mathbb{E}[e^{-t_0 X_n}] &\leq& 1\mathbb{P}(X_n \leq \delta) + 
e^{-t_0 \delta} \mathbb{P}( X_n > \delta) = \mathbb{P}(X_n \leq \delta) 
+ e^{-t_0 \delta} (1- \mathbb{P}(X_n \leq \delta)) \\
&=&  e^{-t_0 \delta} + (1- e^{-t_0 \delta})\mathbb{P}(X_n \leq \delta) 
\eQe
for any $\delta > 0$ and $n \in \mathbb{N}$, and hence
\eQb
\dfrac{1 - \frac{\epsilon}{2} - e^{-t_0 \delta}}{1-e^{-t_0 \delta}} 
&\leq& \mathbb{P}(X_n \leq \delta) \\ 
\eQe
...
\eQb
1- \epsilon &\leq& \mathbb{P}(X_n \leq \delta_0)  
\eQe
for each $n \geq N$. 
...
\eQb
\mathbb{P}(X < 0) &\leq& \liminf_{n \to \infty} \mathbb{P}(X_n < 0) = 0. 
\eQe 
...
and
\eQb
|\mathbb{P}(X_{n_j} \leq x) - \mathbb{P}(X \leq x)| &\geq& \epsilon
\eQe 
...
\eQb
\mathbb{E}[e^{-tX}] = \lim_{k \to \infty} \mathbb{E}[e^{-tX_{n_k}}] 
&=& \lim_{j \to \infty} \mathbb{E}[e^{-tX_{n_j}}] = \mathbb{E}[e^{-tY}]
\eQe 
...
\eQb
\mathbb{E}[e^{-tX}] &=& \lim_{n \to \infty} \mathbb{E}[e^{-tX_n}] = g(t)
\eQe
...
\eQb
\lim_{n \to \infty} \mathbb{E}[e^{-tX_n}] = \exp(\int_{0}^{1} \dfrac{1}{x}
(e^{-xt} - 1) dx)  
\eQe 
for any $t > 0$. Suppose the above limit is established. Observe that 
\eQb
\dfrac{1}{x}(e^{-xt} - 1) \>\> &\text{converges pointwise a.e to}& \>\> 0 \>\> 
\text{as} \>\> t \downarrow 0 \>\> \text{on} \>\> [0,1] 
\eQe
and
\eQb
|\dfrac{1}{x}(e^{-xt}-1)| &=& \dfrac{1}{x}(1-e^{-xt}) \leq \dfrac{1}{x}(xt) \leq t
\eQe
for any $ t > 0$ and $x \in (0,1]$. Therefore, by DCT,
\eQb
\lim_{t \downarrow 0} g(t) &=& \lim_{t \downarrow 0} \exp(\int_{0}^{1} 
\dfrac{1}{x} (e^{-xt} - 1)dx) 
= \exp(\lim_{t \downarrow 0} \int_{0}^{1} \dfrac{1}{x}(e^{-xt}-1)dx) \\
&=& \exp(\int_{0}^{1} \lim_{t \downarrow 0} \dfrac{1}{x}(e^{-xt}-1)dx) = \exp(0) = 1. 
\eQe
We now prove the claimed limit. By independence, 
\eQb
\mathbb{E}[e^{-tX_n}] &=& \mathbb{E}[e^{-t \frac{1}{n} \sum_{ j=1}^{n} j I_j }] 
= \prod_{j=1}^{n} \mathbb{E}[e^{\frac{-tj}{n}I_j}] \\
&=& \prod_{j=1}^{n} (e^{-\frac{tj}{n}}\mathbb{P}(I_j = 1) + 1\mathbb{P}(I_j = 0)) =
\prod_{j=1}^{n} (\dfrac{1}{j} e^{-\frac{tj}{n}} + \dfrac{j-1}{j}) 
\eQe
for any $t > 0$ and $n \in \mathbb{N}$. By Taylor series' approximation of $\log(1-x)
\sim -x$,
\eQb
\lim_{n \to \infty} \mathbb{E}[e^{-tX_n}] &=& \lim_{n \to \infty} 
\exp(\log(\prod_{j=1}^{n} \dfrac{1}{j} e^{-\frac{tj}{n}} + \dfrac{j-1}{j})) 
= \exp(\lim_{n \to \infty} \sum_{j=1}^{n} \log(\dfrac{1}{j} e^{-\frac{tj}{n}} + 
\dfrac{j-1}{j})) \\
&=& \exp(\lim_{n \to \infty} \sum_{j=1}^{n} \log(1 - (
\dfrac{1}{j} - \dfrac{1}{j}e^{-\frac{tj}{n}}))) = \exp(\lim_{n \to \infty} 
\sum_{j=1}^{n} -(\dfrac{1}{j} - \dfrac{1}{j}e^{\frac{-tj}{n}})) \\
&=& \exp(\lim_{n \to \infty} \dfrac{1}{n} \sum_{j=1}^{n} 
\dfrac{n}{j}(e^{
-\frac{tj}{n}} - 1)) = \exp(\int_{0}^{1} \dfrac{1}{x} (e^{-xt} - 1)dx). 
\eQe 
\hfill $\qed$
\end{solution}
